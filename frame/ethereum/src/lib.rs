...
impl<T: Config> Pallet<T> {
	...
	pub fn migrate_block_v0_to_v2() -> Weight {
		...
		if let Some(block_v0) = block_v0 {
			...
			let block_v2: ethereum::BlockV3 = block_v0.into();
			frame_support::storage::migration::put_storage_value::<ethereum::BlockV3>(
				Self::name().as_bytes(),
				item,
				&[],
				block_v2,
	...
	pub fn post_migrate_block_v2(v0_data: Vec<u8>) -> Result<(), &'static str> {
		...
		let item = b"CurrentBlock";
		let block_v2 = frame_support::storage::migration::get_storage_value::<ethereum::BlockV3>(
			Self::name().as_bytes(),
			item,
			&[],
		);
